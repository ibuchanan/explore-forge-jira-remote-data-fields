...
remote_log=$(mktemp "${TMPDIR:-/tmp}/tmp_rovo_remote.XXXXXX")
remote_pid=
tunnel_pid=
...
cleanup() {
	...
	rm -f "$url_file" "$remote_log"
}
trap cleanup EXIT INT TERM

npm run remote:start >"$remote_log" 2>&1 &
remote_pid=$!
...
while kill -0 "$tunnel_pid" 2>/dev/null; do
	if ! kill -0 "$remote_pid" 2>/dev/null; then
		...
		cat "$remote_log" >&2
		printf '%s\n' 'remote exited before the tunnel was ready' >&2
		exit 1
	fi
	[[ -s $url_file ]] && break
	sleep 0.1
...
while kill -0 "$tunnel_pid" 2>/dev/null; do
	if ! kill -0 "$remote_pid" 2>/dev/null; then
		wait "$remote_pid" || true
		cat "$remote_log" >&2
		printf '%s\n' 'remote exited while the tunnel was running' >&2
		exit 1
	fi
	sleep 0.1
done
